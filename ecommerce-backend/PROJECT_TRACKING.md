...
### Phase 7 — Senior/Advanced NestJS (ordered easy → hard)

| #    | Task                                                                                                                                           | Done | Notes                                                                                                                                                                                               |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7.1  | **Event Listeners** — Wire up `@OnEvent()` listeners for `order.created` and `payment.processed` (e.g., send email, update inventory)          | [x]  | Migrated from EventEmitter to BullMQ; `OrderEventsProcessor` (@Processor) handles `order.created`, `order.failed`, `payment.processed` with retry                                                   |
| 7.2  | **Caching** — Apply `@Cacheable()` / `CacheInterceptor` on Products GET endpoints, add Redis store, cache invalidation on create/update        |      | CacheModule registered but unused                                                                                                                                                                   |
| 7.3  | **Serialization** — Custom serializers with `@Expose()`, response mapping strategies                                                           |      |                                                                                                                                                                                                     |
| 7.4  | **Configuration** — Multi-environment config factories, secrets injection, runtime config validation                                           |      |                                                                                                                                                                                                     |
| 7.5  | **Dependency Injection** — Custom providers (`useFactory`, `useExisting`), provider scopes (Request/Transient), `forwardRef` for circular deps |      |                                                                                                                                                                                                     |
| 7.6  | **Dynamic Modules** — Build a custom configurable module (e.g., `forRootAsync()` pattern)                                                      | [x]  | `NotificationsModule` uses `ConfigurableModuleClass` with `.register()` pattern; `MODULE_OPTIONS_TOKEN` re-exported for consumer use                                                                |
| 7.7  | **Advanced TypeORM** — Transactions (`QueryRunner`), subscribers/entity listeners, optimistic locking, soft deletes                            | [x]  | Transactions using QueryRunner in OrdersService, ProductSubscriber for stock changes, @VersionColumn & @DeleteDateColumn in Product, fully migrated                                                 |
| 7.8  | **Advanced Auth** — Refresh tokens, refresh token rotation, OAuth social login                                                                 |      |                                                                                                                                                                                                     |
| 7.9  | **Background Jobs** — Queue-based email sending with BullMQ, retry strategies, dead-letter queues                                              | [x]  | BullMQ (`@nestjs/bullmq`) replaces EventEmitter; `ORDER_QUEUE` used by `OrdersService` + `PaymentsService` as producers; `OrderEventsProcessor` is the worker with `@OnWorkerEvent` lifecycle hooks |
| 7.10 | **Performance** — Profile with clinic.js, optimize DB queries, add compression middleware, streaming                                           |      |                                                                                                                                                                                                     |
| 7.11 | **Comprehensive Testing** — Integration tests with real DB, controller/guard/pipe unit tests, full e2e coverage                                |      |                                                                                                                                                                                                     |
| 7.12 | **Microservices** — Migrate one module to TCP/RabbitMQ transport, hybrid app                                                                   |      | Hardest — major architectural shift                                                                                                                                                                 |

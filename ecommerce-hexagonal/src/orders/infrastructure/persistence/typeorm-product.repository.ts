...
export class TypeOrmOrderProductRepository implements OrderProductRepository {
  ...
  async findById(id: string): Promise<Product | null> {
    ...
    if (!orm) return null;
    return new Product(orm.id, orm.name, orm.description, Number(orm.price), orm.stock, orm.version);
  }

  async updateStock(id: string, newStock: number, version: number): Promise<void> {
